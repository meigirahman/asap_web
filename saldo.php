		...
		<div id="page-wrapper">
                    ...
                    <div class="panel panel-info">
						...
						<span class='glyphicon glyphicon-edit'></span> Tambahkan Data
								...
								<form name="FLaporan" method="post" action="hapus_banyak_skck.php" onsubmit="return confirm('Hapus data terpilih?')" >
                                    ...
                                    <?
                                    $myquery="select * from tb_saldo where kd_opd='$_SESSION[kd_opd]' and kd_sub='$_SESSION[kd_sub]' ";
                                    $no=1;
                                   ...
                                   <?
								   $sql=mysql_query("select sum(jumlah*harga_satuan) as total from tb_saldo_rinci where kd_opd='$_SESSION[kd_opd]' and kd_saldo='$dataku->kd_saldo' group by kd_saldo") or die(mysql_error());
		 $jlh=mysql_fetch_object($sql);
		  ...
		  <td style="text-align:right"><? echo number_format($jlh->total,2,',','.')."</td>"?>
									...
									<td align="center">
                                    <a href='?p=sub_saldo&kd_opd=<?php echo $dataku->kd_opd?>&kd_saldo=<?php echo $dataku->kd_saldo?>&tgl=<?php echo $dataku->tgl?>' class='btn btn-warning btn-sm'>
						<span class='glyphicon glyphicon-edit'>Pilih</span> 
					  </a>
					  
					      <a href='?p=standar_hapus&kd_saldo=<?php echo $dataku->kd_saldo?>' class='btn btn-danger btn-sm'>
