...
<div class="panel-heading">
		...
		<?
	$sql_data   = "select * from tb_saldo_rinci where kd_saldo='$_GET[kd_saldo]' ";
	$mysql_data = mysqli_query($konek, $sql_data) or die ("Query Data Error :" .mysqli_error()); 
	...
	echo "
		  ...
		  echo "Rp " . number_format($jumlah,2,',','.')."</td>";?>
				  ...
				  <td>
					<center>
					    <a href="?p=sub_saldo_edit&kd_saldo=<? echo $_GET['kd_saldo']; ?>&kd_saldo_rinci=<? echo $data['kd_saldo_rinci']; ?>&tgl=<? echo $_GET['tgl']; ?>" class='btn btn-success btn-sm'>
						Edit 
					  </a>
					   <a onclick="return confirm('Yakin Hapus?')" href="?p=sub_saldo_hapus&kd_saldo=<? echo $_GET['kd_saldo']; ?>&kd_saldo_rinci=<? echo $data['kd_saldo_rinci']; ?>&tgl=<? echo $_GET['tgl']; ?>" class='btn btn-danger btn-sm'>
						Hapus 
					  </a>
					</center>
				  </td>
...
<div style='margin-left: 50px' class='col-lg-3'>	
 <div class="panel panel-info">
  ...
  <form role="form" name="tes" method="post" action="home.php?p=sub_saldo_simpan">
			...
			$('#harga_satuan').val(ui.item.harga); 
                                  ...
                                  <div align="center"> 
                                    ...
                                    <input name="fulang" type="reset" id="fulang" value="Reset" class="btn btn-default">
                                     <input name="fulang2" type="button" id="fulang2" value="Kembali" onclick="javascript:location.href='?p=saldo'" class="btn btn-warning">       </div>
