...
<center>Periode <?php echo date("d F Y", strtotime('2021-01-01')) ?> s.d. <?php echo date("d F Y", strtotime($_POST['tgl'])); ?></center></td></tr>	</b> 
  ...
  <?
									 $o="select * from tb_opd where kd_opd='$_SESSION[kd_opd]' and kd_sub!=''";
  ...
  <table border=0 >
		  <tr> <font style="font-size: 14pt; align:left;"><b>Perangkat Daerah : <?php echo $opd->nama?></tr> 
                                    ...
                                    <?
                                    $myquery="select *,sum(jumlah) as jum from tb_stok where kd_opd='$_SESSION[kd_opd]' and kd_sub!='' and tgl between '2021-01-01' and '$_POST[tgl]' group by opsi,kd_sub";
                                    $no=1;
                                    $totm1=0;
                                    $totk1=0;
                                    $tots1=0;
                                   ...
                                   <tr  style="border: 1px solid black;" > 
									...
									<?
									 $awal2="select sum(jumlah) as jumlah from tb_saldo_rinci where kd_brg='$dataku->kd_brg' and kd_opd='$_SESSION[kd_opd]' and kd_sub!='' group by kd_sub,kd_brg ";
								   ...
								   {echo $sawal=0;} ?>
									 ...
									 $a="select * from tb_stok where kd_stok='$dataku->kd_stok' and kd_opd='$_SESSION[kd_opd]' and kd_sub!='' group by kd_sub order by tgl desc limit 1 ";
                                    
                                    $aa=mysql_query($a) or die (mysql_error());
                                   $harga=mysql_fetch_object($aa);
								   ?>
										 <td style="text-align: right;" ><?
									 $mmm="select sum(jumlah) as jlh from tb_stok where kd_stok='$dataku->kd_stok' and opsi='masuk'   group by kd_brg,kd_sub    ";
									 $kkk="select sum(positif) as jlh from tb_stok where kd_stok='$dataku->kd_stok' and opsi='keluar'   group by kd_brg,kd_sub   ";
                                    
                                    $mm=mysql_query($mmm) or die (mysql_error());
                                   $msk=mysql_fetch_array($mm);
								   
								    $kk=mysql_query($kkk) or die (mysql_error());
                                   $klr=mysql_fetch_array($kk);
								   
								   ?>
								   
								   <?php echo number_format($masuk=$msk['jlh'],2,',','.');?></td>  
								   <td  style="text-align: right;"> <?php if($keluar=$klr['jlh']>0) {echo number_format($keluar=$klr['jlh'],2,',','.');}else {echo number_format($klr=0,2,',','.');}?></td> 
...
<?
$t="select * from tb_ttd where kd_opd='$_SESSION[kd_opd]' and kd_sub!='' and jab='upb'";
                                    $tt=mysql_query($t) or die (mysql_error());
                                    $ttd=mysql_fetch_object($tt);
									
									$t2="select * from tb_ttd where kd_opd='$_SESSION[kd_opd]' and kd_sub!='' and jab='pengelola'";
...
<tr><td>&nbsp;</td></tr>									  
<tr><td> </td><td  colspan=2>Disetujui Tanggal, <? echo date('d F Y');?><br><?php echo $ttd->jabatan?></td><td colspan=8></td><td colspan=2><br><?php echo $ttd2->jabatan?></td></tr>
