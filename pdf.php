...
$this->Cell(35,6,'Kode Rekening',1,0,'C',true);
$this->Cell(65,6,'Uraian',1,0,'C',true);
...
$this->Cell(25,6,'Jumlah',1,1,'C',true);
	...
	{
		include 'koneksi.php';
		$map3="SELECT rek4 FROM tb_stok where kd_opd='$_SESSION[kd_opd]' and kd_sub='$_SESSION[kd_sub]' and opsi='masuk' GROUP BY rek4 having sum(jumlah)>0 ORDER BY rek4 ASC";
			...
			while($map=mysqli_fetch_object($map2))
			{
			 ...
			 $this->Cell(35,6,$map->rek4,1,0,'L',true);
			...
			$mapp=mysqli_fetch_object($mapp2);
			 $this->Cell(140,6,$mapp->nm_asap,1,1,'L',true);
			 
			 $this->SetFont('Arial','',8);
		

		$myquery="select * from tb_stok where opsi='masuk' and rek4='$map->rek4' and kd_opd='$_SESSION[kd_opd]' and kd_sub='$_SESSION[kd_sub]' and tgl between '2021-01-01' and '$_POST[tgl]'";
		$no=1;
		$total1=0;
		...
		$this->Cell(35,6,$dataku->kd_brg,1,0);
		$this->Cell(65,6,substr($dataku->uraian,0,38),1,0);
       	$this->Cell(25,6,number_format($dataku->jumlah,2,',','.').'',1,0,'C');
		...
		$this->SetFont('Arial','B',8);
			...
			$total2=$total2+$total1;
				...
				$this->SetFont('Arial','B',8);
         	  $this->SetFillColor(100, 150, 250);  
		$this->Cell(160,6,'TOTAL PEMBELIAN',1,0,'C',true);		
	...
	function Footer()
	{
		...
		//nomor halaman
		$this->Cell(90,6,'ASAP | Daftar Transaksi Pembelian',0,0,'L');
